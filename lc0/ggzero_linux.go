...
var (
	...
	hostname = flag.String("hostname", "http://www.ggzero.cn", "Address of the server")
...
func readSettings(path string) (string, string) {
	...
	if err != nil {
		...
		// 缺省的数据
		//settings.Args = "--threads=2 --minibatch-size=256 --backend=multiplexing --backend-opts=(backend=cudnn,gpu=0) --parallelism=8 --nncache=8000000"

		settings.Args = "--threads=4 --minibatch-size=256 --parallelism=8 --nncache=8000000"

		//
		//settings.DeleteOldWeightsWhenOpen = false
		//settings.DeleteTrainWhenOpen = false
		//settings.IsDownloading = false
		settings.IsSaveTrain = true //
	...
	go func() {
		...
		for stdoutScanner.Scan() {
			...
			case strings.HasPrefix(line, "gameready "):
				...
				idxPgn := strings.LastIndex(line, "pgn")
			...
			case strings.HasPrefix(line, "bestmove "):
				...
				//fmt.Printf("lc0_lgl_bestmove: %s\n", c.BestMove)

			case strings.HasPrefix(line, "id name GGZero "):
...
func playMatch(httpClient *http.Client, ngr client.NextGameResponse, baselinePath string, candidatePath string, params []string) (*client.NextGameResponse, error) {
	...
	if !hasVisitsParam {
		params = append(params, "--visits=400")
	}
...
func getNetwork(httpClient *http.Client, sha string, keepTime string) (string, error) {
	...
	if err != nil {
		if err == lockfile.ErrBusy {
			log.Println("其它客户端正在下载权重，请稍候...")
			return "", err
		} else {
			log.Fatalf("不能加锁文件: %v", err)
		}
	}
	...
	defer lock.Unlock()
	fmt.Println("正在下载权重中...")
	err = client.DownloadNetwork(httpClient, *dls_hostname, path, sha)
	if err != nil {
...
func main() {
	fmt.Printf("GGZeroWDL 训练客户端版本 %v\n", getExtraParams()["version"])
	...
	if *useTestServer {
		*hostname = "http://testserver.ggzero.cn"
	}

	log.SetFlags(log.LstdFlags | log.Lshortfile)
	if len(*user) == 0 || len(*password) == 0 {
		*user, *password = readSettings("trainWDL.json")
	}
