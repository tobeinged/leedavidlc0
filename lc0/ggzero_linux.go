...
var SettingFile = "TrainSetting.json"
var VERSION  = "v0.18"
...
func getNetwork(httpClient *http.Client, sha string, clearOld bool) (string, error) {
	...
	if settings.IsDownloading == true{
		...
		for sum < 1000000 {
			...
			readSettings(SettingFile)
			if settings.IsDownloading == false {
				pathn := filepath.Join("networks", sha)
				return pathn, nil
			}
			sum += 1
		}
	...
	settingsFile, err := os.Create(SettingFile)
	defer settingsFile.Close()
	if err != nil {
		log.Fatal("Could not create output file ", err)
		return "", nil
	...
	// Sha already exists?
	pathn := filepath.Join("networks", sha)
	if stat, err := os.Stat(pathn); err == nil {
		if stat.Size() != 0 {
	...
	// google colab download from dlas http://www.ggzero.cn
	// err = client.DownloadNetwork(httpClient, *hostname, pathn, sha)


	err = client.DownloadNetwork(httpClient, *dls_hostname, pathn, sha)
